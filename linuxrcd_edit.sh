...
find "/media/LiveDiskCreAtionChrootFolDer" -name "*$OriginalText*"  -not -path "/media/LiveDiskCreAtionChrootFolDer/proc/*" -not -path "/media/LiveDiskCreAtionChrootFolDer/sys/*" -not -path "/media/LiveDiskCreAtionChrootFolDer/dev/*" -not -path "/media/LiveDiskCreAtionChrootFolDer/tmp/*" | while read FILEPATH
do
echo "Renaming $FILEPATH"
rename "s/(\W|^)$OriginalText(\W|$)/\1$TargetText\2/g" "$FILEPATH"
done
